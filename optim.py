...
def rmsprop(theta, dtheta, config=None):
  ...
  next_theta = theta - config['learning_rate']/(config['epsilon']+np.sqrt(config['cache']))*dtheta
...
def adam(theta, dtheta, config=None):
  ...
  s = config['m']/(1-config['beta1']**config['t'])
  r = config['v']/(1-config['beta2']**config['t'])
  next_theta = theta - config['learning_rate']*s/(np.sqrt(r) + config['epsilon'])
  pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################

...
def affine_forward(x, theta, theta0):
  ...
  m = x.shape[0]
  out = np.dot(x.reshape(m,-1),theta) + theta0
  pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def affine_backward(dout, cache):
  ...
  dtheta = np.dot(x.reshape(m,-1).T, dout)
...
def conv_forward_naive(x, theta, theta0, conv_param):
  ...
  out = np.zeros((M, F, out_height, out_width))
      ...
      for f in range(F):
          for h in range(out_height):
              for w in range(out_width):
                  out[m,f,h,w] = np.sum(x_padded[m,:,h*stride:h*stride+HH,w*stride:w*stride+WW]*theta[f]) + theta0[f]
      ...
      for f in range(F):
           for h in range(out_height):
              for w in range(out_width):
               # print(w)
               # print(w*S,w*S+WW)
                  dx[m,:,h*stride:h*stride+HH,w*stride:w*stride+WW] += dout[m,f,h,w]*theta[f]
                  dtheta[f] += dout[m,f,h,w]*x_padded[m,:,h*stride:h*stride+HH,w*stride:w*stride+WW]
              #  print(dout[m,f,h,w])
  dx = dx[:,:,p:-p, p:-p]
...
def max_pool_forward_naive(x, pool_param):
  ...
  out_width = np.int((H - pool_width) / stride + 1)
...
def max_pool_backward_naive(dout, cache):
  ...
  for h in range(HH):
    for w in range(WW):
      for n in range(N):
        for c in range(C):
          x_sub = x[n,c,h*stride:h*stride+pool_height,w*stride:w*stride+pool_width]
          #print(x_sub,np.max(x_sub))
          cd = np.where(x_sub == np.max(x_sub))
          dx[n,c,h*stride+cd[0], w*stride+cd[1]] = dout[n,c,h,w]
